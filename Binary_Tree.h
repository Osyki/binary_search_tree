...
public:
    ...
    */
    explicit Binary_Tree(const Item_Type &the_data,
    ...
    virtual ~Binary_Tree() = default; // Do nothing.
...
std::string Binary_Tree<Item_Type>::to_string() const {
    ...
    if (is_null())
        ...
        os << get_right_subtree().to_string(); //recursive
    }
    return os.str();
}
...
read_binary_tree(std::istream &in) {
    ...
    if (next_line == "NULL") {
        ...
        ins >> the_data;
        Binary_Tree<Item_Type> left = read_binary_tree(in); //recursive
        Binary_Tree<Item_Type> right = read_binary_tree(in); //recursive
